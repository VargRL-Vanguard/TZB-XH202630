...
> **你（成员 4/D）负责**：`4_AI集成/`，是**多智能体协同编排** + **审核裁判 Agent** + **3 个 AI provider** + **可视化事件** + **演示视频 + PPT** 的唯一负责区。
...
## 涉及文件
- `backend/4_AI集成/db.py`（新建）— 连接池 + `get_session()` 上下文管理器
- `backend/4_AI集成/config.py`（新建）— 读取 `AI_INTEGRATION_DB_URL` + 3 个 AI 的 `*_API_KEY` / `*_ENDPOINT`
- `backend/4_AI集成/models/`（按表分文件）
...
- `backend/4_AI集成/providers/embed_ai.py`（**挑战杯新增** — AI 4：embedding，用于知识库检索）
...
**依赖**：D-02, B-05, C-04
**优先级**：P0
**归属目录**：`backend/4_AI集成/orchestrator/`（新建）

## 涉及文件
- `backend/4_AI集成/orchestrator/__init__.py`（新建）
- `backend/4_AI集成/orchestrator/pipeline.py`（新建）— **3 Agent 协同编排主体**
- `backend/4_AI集成/orchestrator/debate_engine.py`（**挑战杯核心**）— **辩论与交叉验证引擎**
- `backend/4_AI集成/orchestrator/event_emitter.py`（新建）— 包装 A-04 的 ws.emit，自动落 `agent_log` 表
- `backend/4_AI集成/orchestrator/decision_maker.py`（新建）— 决策选择：pass / retry / 反馈降维
- `backend/4_AI集成/tests/test_orchestrator.py`（新建）
...
- [ ] 在 `backend/4_AI集成/概览.md` 中更新「对外约定」
...
**依赖**：D-02, B-06
**优先级**：P0
**归属目录**：`backend/4_AI集成/audit/`（新建）

## 涉及文件
- `backend/4_AI集成/audit/__init__.py`（新建）
- `backend/4_AI集成/audit/audit_agent.py`（新建）— 审核裁判 Agent 主体
- `backend/4_AI集成/audit/audit_prompts.py`（新建）— 审核 prompt 模板（顶部注释 `version: 0.1`）
- `backend/4_AI集成/audit/groundness_check.py`（新建）— **幻觉率校验**（基于 B 的 `list_kb_chunks_by_kp` + A 的 `calc_hallucination_rate`）
- `backend/4_AI集成/audit/coverage_check.py`（新建）— **核心知识点覆盖率校验**
- `backend/4_AI集成/tests/test_audit_agent.py`（新建）
...
## 涉及文件
- `backend/4_AI集成/ws_bridge.py`（新建）— 把编排器的 event_emitter 桥接到 A-04 的 ws
- `backend/4_AI集成/api/visualization.py`（新建）— **可视化数据接口**（用于前端大屏）
- `backend/4_AI集成/api/trace.py`（新建）— **trace 回放接口**（演示视频用）
- `backend/4_AI集成/tests/test_ws_bridge.py`（新建）
...
## 涉及文件
- `backend/4_AI集成/chat/send.py`（新建）— `/api/ai-chat/send`
- `backend/4_AI集成/chat/history.py`（新建）— `/api/ai-chat/history`
- `backend/4_AI集成/chat/clear.py`（新建）— `/api/ai-chat/history` (DELETE)
- `backend/4_AI集成/path/ai_result.py`（新建）— `/api/learning-path/ai-result`
- `backend/4_AI集成/suggest/ai_result.py`（新建）— `/api/suggestions/ai-result`
- `backend/4_AI集成/models/ai_conversation.py`（新建）
- `backend/4_AI集成/models/ai_result.py`（新建）
...
**依赖**：D-03, D-07, C-08
**优先级**：P0
**归属目录**：`backend/4_AI集成/demo_video/`（新建）→ 最终归档到 `docs/demo_video/`

## 涉及文件
- `backend/4_AI集成/demo_video/script.md`（**先写脚本**）— 10 分钟逐字稿
- `backend/4_AI集成/demo_video/recording_notes.md` — 录制注意事项
- `backend/4_AI集成/demo_video/raw/`（新建）— 原始视频 + 工程文件
- `backend/4_AI集成/demo_video/final.mp4`（**最终交付物**）
...
**依赖**：D-08
**优先级**：P0
**归属目录**：`backend/4_AI集成/ppt/`（新建）→ 最终归档到 `docs/ppt/`

## 涉及文件
- `backend/4_AI集成/ppt/slides.md`（新建）— 逐页大纲
- `backend/4_AI集成/ppt/final.pptx`（**最终交付物**）
...
## 涉及文件
- `backend/4_AI集成/tests/test_e2e_quality.py`（新建）
