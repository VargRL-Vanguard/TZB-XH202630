...
> **你（成员 1/A）负责**：`公共/` + `1_用户与聊天/`，是系统的**地基 + 实时推送通道**，所有 Agent 的过程可视化都依赖你的 WebSocket。
...
## 验收标准（每项必须 ✅，量化）
- [ ] `response.py.ok(data=None, message="success")` / `.fail(code, message)` 输出 `api-doc.js` 中规定的 `{code, message, data}` 结构
- [ ] `logger.py` 支持按模块命名，输出含 `timestamp / level / module / trace_id`
- [ ] `errors.py` 定义 5 类异常：`BizError` / `AuthError` / `NotFoundError` / `AgentError`（Agent 调度失败） / `QualityError`（指标不达标）
- [ ] `config.py` 导出 `PORT` / `LOG_LEVEL` / `ENV` / `JWT_SECRET` / `AGENT_TIMEOUT_SEC=30` / `QUALITY_THRESHOLD`；**不**包含任何 `*_DB_URL`
- [ ] **`metrics.py` 提供 3 个纯函数**（挑战杯核心指标）：
  - `calc_hallucination_rate(generated: str, ground_truth: list[str]) -> float` — 返回 [0,1] 浮点，**必须 < 0.05**
  - `calc_match_accuracy(profile: dict, resource_difficulty: int) -> float` — 返回 [0,1] 浮点，**必须 ≥ 0.85**
  - `calc_coverage(generated: str, required_kps: list[str]) -> float` — 返回 [0,1] 浮点，**必须 ≥ 0.90**
  - 每个函数有 ≥ 3 个单测（含边界值 0、1、空输入）
- [ ] `__init__.py` 全部导出，B/C/D 可 `from backend.公共 import response, logger, errors, metrics`
- [ ] 在群里公告 `config.py` 字段清单 + 3 个指标函数的输入输出签名
...
## 描述
为 B / C / D 提供统一的"解析 token → userId"能力，**所有业务接口**都依赖它。鉴权过程中需要查 `1_用户与聊天/` 的 User 表，通过 A 暴露的 `get_user_by_id` 函数读取（**不**直连 A 的数据库）。
...
## 涉及文件
- `backend/1_用户与聊天/db.py`（新建）— 连接池 + `get_session()` 上下文管理器
- `backend/1_用户与聊天/config.py`（新建）— 读取 `USER_CHAT_DB_URL`
- `backend/1_用户与聊天/models/`（按表分文件）
...
## 涉及文件
- `backend/1_用户与聊天/auth/register.py`（新建）— bcrypt 哈希密码
- `backend/1_用户与聊天/auth/login.py`（新建）— 签发 JWT
- `backend/1_用户与聊天/auth/logout.py`（新建）— token 失效
...
- `backend/1_用户与聊天/user/profile.py`（新建）— **挑战杯新增** 学习者画像读写
- `backend/1_用户与聊天/models/learner_profile.py`（新建）— LearnerProfile 表
- `backend/1_用户与聊天/tests/test_profile.py`（新建）
...
## 涉及文件
- `backend/1_用户与聊天/chat/{send,history,list,read}.py`
- `backend/1_用户与聊天/models/message.py`
- `backend/1_用户与聊天/tests/test_chat.py`
...
- `backend/1_用户与聊天/ws/events.py`（新建）— **挑战杯新增**：Agent 协同事件协议
- `backend/1_用户与聊天/tests/test_ws.py`（新建）
