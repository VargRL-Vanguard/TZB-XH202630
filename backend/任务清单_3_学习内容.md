...
> **你（成员 3/C）负责**：`3_学习内容/`，是**领域专家 Agent** + **3 种形态资源（定制化资源 / 实操指南 / 分阶测试题）**的唯一负责区。
...
## 涉及文件
- `backend/3_学习内容/db.py`（新建）— 连接池 + `get_session()` 上下文管理器
- `backend/3_学习内容/config.py`（新建）— 读取 `LEARNING_CONTENT_DB_URL`
- `backend/3_学习内容/models/`（按表分文件）
...
**依赖**：C-00, B-05
**优先级**：P0
**归属目录**：`backend/3_学习内容/agents/`（新建）

## 涉及文件
- `backend/3_学习内容/agents/__init__.py`（新建）
- `backend/3_学习内容/agents/expert_agent.py`（新建）— 领域专家 Agent 主体
- `backend/3_学习内容/agents/expert_prompts.py`（新建）— 领域专家 prompt 模板（顶部注释 `version: 0.1`）
- `backend/3_学习内容/agents/resource_factory.py`（新建）— 3 种形态资源工厂
- `backend/3_学习内容/agents/kp_coverage_check.py`（新建）— **核心知识点覆盖率校验**（对接 A 的 `公共/metrics.py`）
- `backend/3_学习内容/tests/test_expert_agent.py`（新建）
...
- [ ] 在 `backend/3_学习内容/概览.md` 中更新「对外约定」
...
## 涉及文件
- `backend/3_学习内容/learning_path/renderers/customized_resource.py`（新建）
- `backend/3_学习内容/learning_path/renderers/practice_guide.py`（新建）
- `backend/3_学习内容/learning_path/renderers/tiered_quiz.py`（新建）
- `backend/3_学习内容/learning_path/renderers/__init__.py`（新建）
...
- [ ] **新增** `from backend.3_学习内容.agents import generate_resource`（C-04）
...
## 涉及文件
- `backend/3_学习内容/learning_path/feedback_adapter.py`（新建）
- `backend/3_学习内容/models/interaction_log.py`（新建）— 交互记录表（question_id / correct / response_time / kp_id）
- `backend/3_学习内容/tests/test_feedback_adapter.py`（新建）
...
## 涉及文件
- `backend/3_学习内容/tests/test_e2e_quality.py`（新建）
...
**归属目录**：`backend/3_学习内容/sample_resources/`（新建）

## 涉及文件
- `backend/3_学习内容/sample_resources/customized_resource_sample.html`
- `backend/3_学习内容/sample_resources/practice_guide_sample.html`
- `backend/3_学习内容/sample_resources/tiered_quiz_sample.html`
- `backend/3_学习内容/sample_resources/README.md`（**脱敏声明** + 选用画像说明）
